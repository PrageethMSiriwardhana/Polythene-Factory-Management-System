...
export default function TotalOTCard001() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Calculate the total OT from the response data
                const totalOTFromAPI = response.data.reduce((total, item) => total + item.totalOT, 0);
                setTotalOT(totalOTFromAPI);
            } catch (error) {
                console.error('Failed to fetch total OT:', error);
            }
        };
        fetchData();
    }, []);
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-600 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-700">
            <div className='flex'>
            <svg className='' width="60px" height="60px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000" strokeWidth="0.00024">
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Total OT</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {totalOT.toFixed(2)}</p>
