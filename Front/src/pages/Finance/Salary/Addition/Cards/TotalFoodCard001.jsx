...
export default function TotalFoodCard001() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Calculate the total allowance from the response data
                const totalAllowanceFromAPI = response.data.reduce((total, item) => total + item.totalAllowance, 0);
                setTotalAllowance(totalAllowanceFromAPI);
            } catch (error) {
                console.error('Failed to fetch total allowance:', error);
            }
        };
        fetchData();
    }, []);
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-600 border border-gray-200 rounded-lg shadow w-100 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800">
            <div className='flex'>
            <svg className='' width="60px" height="60px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000" strokeWidth="0.00024">
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Food Allowance</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {totalAllowance.toFixed(2)}</p>
            <Link to='/finance?tab=monthfoodAllwance' className="inline-flex items-center ml-3 font-medium text-white hover:underline hover:text-slate-100">
                See Tables
