...
export default function AddRolecomp({ onClose }) {
  ...
  const validate = () => {
    ...
    if (!timeIncome) {
      errors.timeIncome = "Please enter the date income.";
      isValid = false;
    } else if (!/^\d*\.?\d*$/.test(timeIncome)) {
      errors.timeIncome = "Please enter a valid number.";
      isValid = false;
    }
  ...
  const handleTimeIncomeChange = (e) => {
  ...
  const handleConfirmSave = async () => {
    try {
      const response = await fetch('http://localhost:3000/salary/addroleotincome', {
        method: 'POST',
        headers: {
        ...
        >
          ...
          <Modal.Body>
            <div className="space-y-6">
              <h3 className="text-xl font-medium text-gray-900 dark:text-white">Add New Role OT Income</h3>
              ...
              <div className="max-w-md">
                ...
                <TextInput id="timeIncome" type="text" placeholder="Enter OverTime Income" value={timeIncome} onChange={handleTimeIncomeChange} />
