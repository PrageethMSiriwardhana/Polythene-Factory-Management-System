...
export default function AllMonthDeductionCard002() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Assuming response.data is an array of objects with allTotalDeduction property
                // Set allTotalDeduction
                const totalDeduction = response.data.reduce((total, item) => total + item.allTotalDeduction, 0);
                setAllTotalDeduction(totalDeduction);
            } catch (error) {
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            ...
            <svg className='' width="60px" height="60px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000" strokeWidth="0.00024">
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">All Deduct</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {allTotalDeduction.toFixed(2)}</p>
            <Link to='/your-tables-route' className="inline-flex items-center ml-3 font-medium text-white hover:underline hover:text-slate-100">
                See Tables
