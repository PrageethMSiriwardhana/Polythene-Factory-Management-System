...
export default function DeductionCard001() {
    ...
    const totalDeduction = deductions.reduce((total, item) => total + item.totalDeduction, 0);
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            <div className='flex'>
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Deduction</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {totalDeduction.toFixed(2)}</p>
            <Link to='/finance?tab=deductiondash' className="inline-flex items-center ml-3 font-medium text-white hover:underline hover:text-slate-100">
                See Tables
