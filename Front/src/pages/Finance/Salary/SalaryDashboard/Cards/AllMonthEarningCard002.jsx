...
export default function AllMonthEarningCard002() {
    const [earnings, setEarnings] = useState([]); // Declare and initialize the earnings state

    const [allTotalEarning, setAllTotalEarning] = useState(0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Calculate allTotalEarning
                const totalEarning = response.data.reduce((total, item) => total + item.allTotalEarning, 0);
                setAllTotalEarning(totalEarning);
            } catch (error) {
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            ...
            <svg className='' width="60px" height="60px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000" strokeWidth="0.00024">
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">All Earning</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {allTotalEarning.toFixed(2)}</p>
