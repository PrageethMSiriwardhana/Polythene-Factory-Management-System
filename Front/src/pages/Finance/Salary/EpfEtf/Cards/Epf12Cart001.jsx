...
export default function Epf12Cart001() {
    // States for basic salaries and EPF8
    const [epf12, setEPF12] = useState(0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Calculate EPF12
                const epf12Total = response.data.reduce((total, item) => total + item.epf12, 0);
                setEPF12(epf12Total);
            } catch (error) {
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            <div className='flex'>
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Epf 12%</label>
            </div>
    
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {epf12.toFixed(2)}</p>
