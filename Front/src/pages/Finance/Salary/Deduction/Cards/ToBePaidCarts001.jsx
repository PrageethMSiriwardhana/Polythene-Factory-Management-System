...
export default function ToBePaidCarts001() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setLoans(response.data); // Update state with fetched data
            } catch (error) {
    ...
    // Calculate total ToBePaid amount from loan data
    const totalToBePaid = loans.reduce((total, item) => total + item.toBePaid, 0);
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            <div className='flex'>
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">To Be Paid</label>
            </div>
            {/* Display total ToBePaid amount */}
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {totalToBePaid.toFixed(2)}</p>
