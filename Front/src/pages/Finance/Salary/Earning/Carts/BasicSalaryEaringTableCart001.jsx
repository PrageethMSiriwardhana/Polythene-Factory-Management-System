...
export default function BasicSalaryEarningTableCart001() {
    // Basic salaries state
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setBasicSalaries(response.data); // Update state with fetched data
            } catch (error) {
    ...
    const totalBasicSalary = basicSalaries.reduce((total, item) => total + item.basicSalary, 0);

    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-green-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            <div className='flex'>
            <svg className='' width="60px" height="60px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000" strokeWidth="0.00024">
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Basic Salary</label>
            </div>
