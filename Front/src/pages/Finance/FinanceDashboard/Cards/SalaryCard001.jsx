...
export default function SalaryCard001() {
    const [netPays, setNetPays] = useState([]);

    useEffect(() => {
        const fetchDataCard001 = async () => {
            try {
    ...
    // Define filteredNetPays or remove the reference if not needed
    const totalNet = netPays.reduce((total, item) => total + item.netTotal, 0);

    return (
        
        <div className="h-40 max-w-sm p-4 bg-green-600 border border-gray-200 rounded-lg shadow w-72 dark:bg-green-800 dark:border-gray-100 hover:bg-blue-70">
            <div className='flex'>
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Net Pay</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {totalNet.toFixed(2)}</p>
