...
const indexTo = currentPage * rowsPerPage;
 ...
 return (
        <div className="biodata-container">
            <h2 className="pt-2 pl-1 text-3xl text-black ">Profiles</h2>
            <div className='flex items-center mt-5 mb-2'>
                <button onClick={handleAdd} className="px-4 py-2 font-bold text-white bg-green-600 rounded hover:bg-green-700">
                    Add Profile
                </button>
                ...
                <tbody className="text-xs text-black uppercase bg-[#cdf8da] border-b border-[#4bf885]">
                    {filteredProfiles.slice(indexFrom, indexTo).map((biodata) => (
                        <tr key={biodata.userId} className="biodata-item hover:bg-[#a1f0c6]">
                            ...
                            <img src={biodata.imgSrc} alt="Profile" className="object-cover w-12 h-12 border-4 rounded-full" onChange={handleUploadImage}/>
                            </td>
                            <td className="px-6 py-4">
                                <button className="px-2 py-1 mr-2 text-white bg-green-500 rounded" onClick={() => handleEdit(biodata.id)}>Edit</button>
                                <button className="px-2 py-1 text-white bg-red-500 rounded" onClick={() => handleRemove(biodata.id)}>Remove</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
