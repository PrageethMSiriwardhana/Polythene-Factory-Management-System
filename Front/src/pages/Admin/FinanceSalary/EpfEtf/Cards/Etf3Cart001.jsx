...
export default function Etf3Cart001() {
    // States for basic salaries and EPF8
    const [etf3, setETF3] = useState(0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const etf3Total = response.data.reduce((total, item) => total + item.etf3, 0);
                setETF3(etf3Total);
            } catch (error) {
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            <div className='flex'>
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Etf 3%</label>
            </div>
    
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {etf3.toFixed(2)}</p>
