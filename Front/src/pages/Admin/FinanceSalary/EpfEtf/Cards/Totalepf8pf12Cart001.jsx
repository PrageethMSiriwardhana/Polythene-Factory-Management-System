...
export default function Totalepf8epf12Cart001() {
    // State for total EPF8 + EPF12
    const [totalEpf8Epf12, setTotalEpf8Epf12] = useState(0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Calculate the total of totalEpf8Epf12 from the response data
                const total = response.data.reduce((acc, item) => acc + item.totalEpf8Epf12, 0);
                setTotalEpf8Epf12(total);
            } catch (error) {
                console.error('Failed to fetch total EPF8 and EPF12:', error);
            }
        };
        fetchData();
    }, []);
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-700 border border-gray-200 rounded-lg shadow w-72 dark:bg-blue-600 dark:border-gray-100 hover:bg-green-800 dark:hover:bg-blue-70 transition duration-300 ease-in-out transform hover:scale-105">
            ...
            <svg className='' width="60px" height="60px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#000000" strokeWidth="0.00024">
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Total 8%12%</label>
            </div>
    
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {totalEpf8Epf12.toFixed(2)}</p>
