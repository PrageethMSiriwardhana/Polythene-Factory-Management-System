...
export default function EditFoodAllowance({ onClose, id }) {
  ...
  const fetchFoodAllowance = async () => {
    try {
      const response = await fetch(`http://localhost:3000/salary/showusertotalloan/${id}`);
      const data = await response.json();
      if (response.ok) {
        const userTotalLoan = data.userTotalLoan;
        setUserId(userTotalLoan.userId);
        setName(userTotalLoan.name);
        setLoanDate(userTotalLoan.loanDate.substring(0, 10));
        setLoanAmount(userTotalLoan.loanAmount);
        setToBePaid(userTotalLoan.toBePaid);
        setLoanRatePercentage(userTotalLoan.loanRatePresentage);
        setLoanDuration(userTotalLoan.loanDuration);
      } else {
  ...
  const handleConfirmSave = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/salary/updateusertotalloan/${id}`, {
        method: 'PUT',
        headers: {
      ...
      setErrors(prevErrors => ({ ...prevErrors, save: 'Failed to save loan' }));
    }
  };
  ...
  return (
    <>
      ...
      {errors.save && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2">
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20, transition: { duration: 0.8, ease: "easeInOut" } }} transition={{ duration: 0.6, ease: "easeInOut" }}>
            <Alert color="failure" icon={HiInformationCircle} onDismiss={() => setErrors(prevErrors => ({ ...prevErrors, save: null }))}>
              Error: {errors.save} ❌
            </Alert>
          </motion.div>
        </div>
