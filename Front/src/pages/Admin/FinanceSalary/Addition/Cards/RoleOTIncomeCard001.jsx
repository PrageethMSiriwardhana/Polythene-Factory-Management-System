...
export default function RoleOTIncomeCard001() {
    const [timeIncome, setTimeIncome] = useState(0);

    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch data from the new API endpoint for time income
                const responseTimeIncome = await axios.get('http://localhost:3000/salary/showroleotincome');

                // Calculate the timeIncome from the response data
                const timeIncomeTotal = responseTimeIncome.data.reduce((acc, curr) => acc + curr.timeIncome, 0);
                setTimeIncome(timeIncomeTotal);
            } catch (error) {
    ...
    return (
        <div className="h-40 max-w-sm p-4 bg-green-600 border border-gray-200 rounded-lg shadow w-72 dark:bg-green-600 dark:border-gray-100 hover:bg-green-800">
            <div className='flex'>
                ...
                <label className="mt-4 ml-3 text-3xl font-semibold text-white dark:text-white">Time Income</label>
            </div>
            <p className="mb-2 ml-3 text-2xl font-normal text-white dark:text-white">Rs: {timeIncome.toFixed(2)}</p>
