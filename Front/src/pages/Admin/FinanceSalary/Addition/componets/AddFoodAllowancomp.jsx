...
export default function AddFoodAllowance({ onClose }) {
  ...
  const validate = () => {
    ...
    if (!allowanceDate) {
      errors.allowanceDate = "Please select a date.";
      isValid = false;
    }

    if (!allowance) {
      errors.allowance = "Please enter the amount.";
      isValid = false;
    } else if (!/^\d*\.?\d*$/.test(allowance)) {
      errors.allowance = "Please enter a valid number.";
      isValid = false;
    }
  ...
  const handleConfirmSave = async () => {
    try {
      console.log('Saving data...', allowanceDate, allowance);
      const response = await fetch('http://localhost:3000/salary/addfoodallowance', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to save food allowance');
      }
      ...
      console.error('Error saving food allowance:', error);
    }
  };
        ...
        >
          ...
          <Modal.Body>
            <div className="space-y-6">
              <h3 className="text-xl font-medium text-gray-900 dark:text-white">Add Food Allowance</h3>
              ...
              <div className="max-w-md">
                ...
                <TextInput id="allowance" type="text" placeholder="Enter allowance amount" value={allowance} onChange={handleAllowanceChange} />
      ...
      {alertVisible && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2">
          ...
          >
            ...
            >
              Food allowance <b>saved successfully!</b> ✅
            </Alert>
          </motion.div>
        </div>
      )}
